...
public class ServerProtocol {
    ...
    public Message processInput(Message theInput) {
        ...
        if(state == WAITING) {
            ...
            else if (theInput.getMessage().equals("6"))
              {
                  state = IN_CONVERSATION;
        ...
        // Retrieve the ride from main list and give it to passenger
        else if (state == GET_ON_RIDE)
        {
            state = IN_CONVERSATION;
            Ride forPassenger = MultiServer.getRideList().getRide(theInput.getMessage());
            //Check if not found then return with invalid input
            if (forPassenger==null)
            {
                state = GET_ON_RIDE;
                theOutput = new Message("Server","Wrong ID - Please Try Again",GET_ON_RIDE);
            }
            //If found, respond success
            else{
